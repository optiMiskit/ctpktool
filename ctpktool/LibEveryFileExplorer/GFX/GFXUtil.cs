	...
	public class GFXUtil
	{
		...
		public static uint ConvertColorFormat(uint InColor, ColorFormat InputFormat, ColorFormat OutputFormat)
		{
			...
			else
			{
                ...
                A = ((((InColor >> InputFormat.AShift) & mask) * 255u) + mask) / mask - 1;
			}
			mask = ~(0xFFFFFFFFu << InputFormat.RSize);
            R = ((((InColor >> InputFormat.RShift) & mask) * 255u) + mask) / mask - 1;
			mask = ~(0xFFFFFFFFu << InputFormat.GSize);
            G = ((((InColor >> InputFormat.GShift) & mask) * 255u) + mask) / mask - 1;
			mask = ~(0xFFFFFFFFu << InputFormat.BSize);
            B = ((((InColor >> InputFormat.BShift) & mask) * 255u) + mask) / mask - 1;
